...
export function SystemOperations() {
  ...
  if (health.isLoading || activation.isLoading) return <div className="workspace-loading">Loading operational controls…</div>
  if (health.error || activation.error) {
    const error = health.error ?? activation.error
    return <section className="state-panel" role="alert"><h1>Operations unavailable</h1><p>{error?.message ?? 'The operational status could not be loaded.'}</p></section>
  }
  if (!health.data || !activation.data) return <section className="state-panel"><h1>Operations unavailable</h1></section>
